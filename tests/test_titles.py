...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            r"\subsection*{the quick and the dead}",
            r"\subsection*{The Quick and the Dead}",
        ),
        ...
        (
            r"\section{the deviations space $D_{g}^{\gls{num}}$}",
            r"\section{The Deviations Space $D_{g}^{\gls{num}}$}",
        ),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        (
            r"\section{\textit{italic} and normal}",
            r"\section{\textit{italic} and Normal}",
        ),
        (
            r"\section{calculating $E=mc^2$ and more}",
            r"\section{Calculating $E=mc^2$ and More}",
        ),
        (
            r"\section{the \textbf{great} filter}",
            r"\section{The \textbf{great} Filter}",
        ),
    ],
)
...
@pytest.mark.xfail(
    reason="Safeguard fails if math is attached mid-word or ends without space."
)
